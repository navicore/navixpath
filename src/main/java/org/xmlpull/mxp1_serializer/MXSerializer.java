...
public class MXSerializer implements XmlSerializer {
    ...
    private void writeNamespaceDeclarations() throws IOException
    {
        ...
        //check for ns len to allow for xmpp stanzas that are of default ns not not specify so //blazebot
        for (int i = elNamespaceCount[ depth - 1 ]; i < namespaceEnd; i++) { //blazebot
            if (namespaceUri[i] != null && namespaceUri[i].length() > 0) {
                if (namespacePrefix[ i ] != "") {
                    out.write(" xmlns:");
    ...
    protected void writeAttributeValue(String value, Writer out) throws IOException
    {
        ...
        for (int i = 0; i < value.length(); i++) {
            ...
            if (ch == '&') {
                ...
                if (ch > 0) {
                    ...
                    //else if(ch == 0) {
                    throw new IllegalStateException("character zero is not allowed in output"+getLocation());
                }
            }
        }
        if (pos > 0) {
        ...
        //                }
        //                break;
        //            } else if(posQuot != -1  //TODO check dead expr optimization
        //                          && (posAmp == -1 || (posAmp != -1 && posQuot < posAmp))
        //                          && (posLt == -1 || (posLt != -1 && posQuot < posLt))
        //                     )
        //            {
        //                if(pos < posQuot) out.write(value.substring(pos, posQuot));
        //                out.write(quotEntity);
        //                pos = posQuot + 1;
        //                posQuot = value.indexOf(quot, pos);
        //            } else if(posAmp != -1
        //                          && (posQuot == -1 || (posQuot != -1 && posAmp < posQuot))
        //                          && (posLt == -1 || (posLt != -1 && posAmp < posLt))
        //                     )
        //            {
        //                if(pos < posAmp) out.write(value.substring(pos, posAmp));
        //                out.write("&amp;");
        //                pos = posAmp + 1;
        //                posAmp = value.indexOf('&', pos);
        //            } else if(posLt != -1
        //                          && (posQuot == -1 || (posQuot != -1 && posLt < posQuot))
        //                          && (posAmp == -1 || (posAmp != -1 && posLt < posAmp))
        //                     )
        //            {
        //                if(pos < posLt) out.write(value.substring(pos, posLt));
        //                out.write("&lt;");
        //                pos = posLt + 1;
        //                posLt = value.indexOf('<', pos);
        //            } else {
        //                throw new IllegalStateException("wrong state #1 posLt="+posLt+" posAmp="+posAmp
        //                                                    +" posQuot="+posQuot+" for "+value);
        //            }
        //        }
    }
    ...
    protected void writeElementContent(String text, Writer out) throws IOException
    {
        ...
        for (int i = 0; i < text.length(); i++) {
            ...
            if (ch == ']') {
                ...
                if (ch != 10 && ch != 9) { // \n and \t can pass through
                    if (ch >0) {
                        //in XML 1.0 only legal are #x9 | #xA | #xD
        ...
        //TODO: check if doing text.get(i) would be faster than copy ...

        //        int pos = 0;
        //        int len = text.length();
        //        while( pos < len) {
        //          int end = pos + buf.length;
        //          if(end > len) end = len;
        //          text.getChars(pos, end, buf, 0);
        //          writeElementContent(buf, 0, end - pos, out);
        //          pos = end;
        //        }
        ...
        //        int posAmp = -1;
        //        if(posLt == -1) posAmp = text.indexOf('&');
        //        int posCR = -1;
        //        if(posAmp == -1) posCR = text.indexOf('\r');
        //        int posCdSect = -1;
        //        if(posCR == -1) posCdSect = text.indexOf("]]>");
        //        // painful loop ...
        //        if(posLt == -1 && posAmp == -1 && posCR == -1 && posCdSect == -1) {
        //            out.write(text);
        //        } else {
        //              int pos = 0;
        //              int len = text.length();
        ...
        //            {
        //                if(pos < posAmp) out.write(text.substring(pos, posAmp));
        //                out.write("&amp;");
        //                pos = posAmp + 1;
        ...
        //            {
        //                if(pos < posLt) out.write(text.substring(pos, posLt));
        //                out.write("&lt;");
        //                pos = posLt + 1;

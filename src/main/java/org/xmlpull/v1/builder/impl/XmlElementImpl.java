...
public class XmlElementImpl implements XmlElement
{
        ...
        if(parent != null) {
            ...
            if(parent instanceof XmlElement) {
                ...
                if(doc.getDocumentElement() != this) {
                    throw new XmlBuilderException(
                        "this element must be root docuemnt element to have document set as parent"
    ...
    public XmlAttribute addAttribute(XmlAttribute attributeValueToAdd) {
        ...
        //TODO: check that htere is no attribute duplication (including namespaces?)
    ...
    public XmlElement element(int position) {
        ...
        if(position > 0 && position <= length) {
            Object child = children.get(position - 1);
            if(child instanceof XmlElement) {
                ++count;
                if(count == position) {
    ...
    public XmlElement element(XmlNamespace n, String name, boolean create) {
        XmlElement e = findElementByName(n.getNamespaceName(), name);
        if(e != null) {
            return e;
        }
